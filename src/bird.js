class Bird {
  constructor() {
    //картинки птички из спрайта
    this.frames = [
      { spriteX: 276, spriteY: 113 },
      { spriteX: 276, spriteY: 141 },
      { spriteX: 276, spriteY: 163 },
    ];
    this.size = [32, 26];
    //расположение на канвасе
    this.x = 60;
    this.y = 200;
    this.radius = 12;

    //для анимации
    this.frame = 0;
    this.direction = 1;

    this.gravity = 0.3;
    this.speed = 3;
    this.jump = 0;
    this.rotation = 0;
    this.period = 0;
  }

  drawEngine() {
    let bird = this.frames[this.frame];

    context.save(); //стабилизируем канвас

    context.translate(this.x, this.y); //переводим начало координат на положение птички
    context.rotate(this.rotation);
    context.drawImage(
      sprite,
      bird.spriteX,
      bird.spriteY,
      this.size[0],
      this.size[1],
      -this.size[0] / 2,
      -this.size[1] / 2, //центрируем положение птички
      this.size[0],
      this.size[1]
    );

    context.restore(); // восстанавливаем состояние
  }

  //обновляем свойства птицы в соответствии с текущим состоянием игры
  update() {
    this.period = state.current === state.gReady ? 6 : 4; //медленно машет
    this.frame += frames % this.period == 0 ? 1 : 0;
    this.frame = this.frame % this.frames.length;

    if (state.current === state.gReady) {
      this.y = 200;
      this.rotation = 0 * DEGREE;
    } else {
      this.downfall();
      if (this.jump >= this.speed) {
        this.rotation = 90 * DEGREE;
        this.frame = 0;
      } else {
        this.rotation = -25 * DEGREE;
      }
    }
  }
  //птичка падает
  downfall() {
    this.jump += this.gravity;
    this.y += this.jump;
    this.fgContact();
  }

  //птичка поднимается
  uplift() {
    this.jump = -this.speed;
  }

  //столкновение с землей -игра окончена
  fgContact() {
    if (this.y + this.size[1] / 2 >= fg.fg.y) {
      this.y = fg.fg.y - this.size[1] / 2;
      if (state.current === state.game) {
        state.current = state.gOver;
        DIE.play();
      }
    }
  }

  clickBird() {
    if (bird.y - bird.radius > 0) {
      // Если птица не достигла верхней границы экрана, то "поднять" птицу
      bird.uplift();
      FLAP.play();
    }
  }

  speedReset() {
    this.jump = 0;
  }
}
